...
class RegressionLayer(M.Module):
    def __init__(self, input = None, target = None, regularize = True):
        ...
        # INTERFACE METHODS
        self.update = M.Method([input, target], [self.cost, self.grad_w, self.grad_b, grad_act],  updates={self.w: self.w - self.stepsize * self.grad_w, self.b: self.b - self.stepsize * self.grad_b})
        self.apply = M.Method(input, self.prediction)
    def params(self):
        return self.w, self.b
    def _instance_initialize(self, obj, input_size = None, target_size = None, seed = 1827, **init):
